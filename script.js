const content = {
  ar: {
    ...
    servicesTag: "ما نقدمه في العيادة",
    ...
    ctaTag: "الحجز السريع",
    ctaTitle: "هل تحتاج موعداً؟",
    ctaText: "يمكنك الحجز مباشرة عبر واتساب أو الاتصال بالعيادة لتحديد الموعد المناسب.",
    ...
    services: [
      ...
      { title: "اضطرابات القلق", text: "التعامل مع القلق العام ومظاهره المختلفة بخطة علاجية متدرجة.", icon: "😌" },
      { title: "نوبات الهلع", text: "تقييم الأعراض ووضع خطة للمساعدة على استعادة السيطرة والشعور بالأمان.", icon: "⚡" },
      { title: "الوسواس القهري", text: "مقاربة علاجية تجمع بين الفهم السريري والأساليب النفسية الحديثة.", icon: "🔄" },
      { title: "اضطرابات النوم", text: "تشخيص صعوبات النوم ومتابعة الأسباب النفسية والسلوكية المرتبطة بها.", icon: "🌙" },
      { title: "اضطرابات المزاج", text: "متابعة التغيرات المزاجية ووضع خطة علاجية مناسبة لكل حالة.", icon: "📈" },
      { title: "الضغوط النفسية", text: "المساعدة على فهم مسببات الضغط النفسي وطرق التعامل معه بصورة صحية.", icon: "🛡️" },
      { title: "بعض اضطرابات الأعصاب", text: "تقييم بعض الأعراض العصبية المرتبطة بالاختصاص ضمن إطار مهني متخصص.", icon: "🧬" },
      { title: "الصداع والشقيقة", text: "متابعة حالات الصداع والشقيقة ضمن التقييم الطبي المناسب.", icon: "💢" }
    ],
    certificates: {
      academic: [
        ...
        { title: "علاج الإدمان على المخدرات", text: "برنامج تدريبي - الأردن.", meta: "برنامج تخصصي" },
        { title: "التحليل النفسي للأطفال والمراهقين - كاليفورنيا", text: "شهادة منفصلة ضمن مجال التحليل النفسي للأطفال والمراهقين.", meta: "شهادة تخصصية" },
        { title: "التحليل النفسي للأطفال والمراهقين - هامبورغ", text: "شهادة منفصلة ضمن المجال نفسه، إضافة إلى شهادة كاليفورنيا.", meta: "شهادة تخصصية" },
    ...
    gallery: [
      ...
      { title: "مؤتمر مكافكة المخدرات", text: "مؤتمر مكافحة المخدّرات في دولة ألمانيا", image: "assets/Gallery_4.jpeg" },
      ...
      { title: "إحتفاليةمستشفى الرشاد", text: "الإحتفاليّة السنويّة ــ تأهيل مرضى مستشفى الرشاد", image: "assets/Gallery_6.jpeg" },
      { title: "مستشفى متروبولوتن ", text: "مرحلة الدراسة في كالفورنيا ـ مستشفى متروبولوتن 2012", image: "assets/Gallery_7.jpeg" }
    ],
    faq: [
  ...
  en: {
    ...
    servicesTag: "What the clinic offers",
    ...
    ctaTag: "Quick Booking",
    ctaTitle: "Need an appointment?",
    ctaText: "You can book directly via WhatsApp or call the clinic to arrange a suitable appointment.",
    ...
    services: [
      ...
      { title: "Anxiety Disorders", text: "Support for generalized anxiety and related symptoms through a structured approach.", icon: "😌" },
      { title: "Panic Attacks", text: "Evaluation of symptoms and a plan to help restore control and a sense of safety.", icon: "⚡" },
      { title: "Obsessive-Compulsive Disorder", text: "A clinical approach combining psychiatric evaluation with modern therapeutic techniques.", icon: "🔄" },
      { title: "Sleep Disorders", text: "Assessment of sleep difficulties and related behavioral or psychological factors.", icon: "🌙" },
      { title: "Mood Disorders", text: "Follow-up for mood changes with a suitable plan for each case.", icon: "📈" },
      { title: "Psychological Stress", text: "Helping patients understand sources of stress and deal with them in a healthier way.", icon: "🛡️" },
      { title: "Selected Neurological Conditions", text: "Assessment of selected neurological symptoms within the clinic's professional scope.", icon: "🧬" },
      { title: "Headache & Migraine", text: "Follow-up for headache and migraine cases within appropriate medical evaluation.", icon: "💢" }
    ],
    certificates: {
      academic: [
...
function setStaticText(lang) {
  ...
  copyright.textContent = content[lang].copyright(new Date().getFullYear());
}

function renderTrust(lang) {
  trustGrid.innerHTML = content[lang].trust.map(item => `
...
function renderMethods(lang) {
  methodList.innerHTML = content[lang].methods.map(item => `
...
function createInteractiveCard(item, className) {
  return `
    <article class="${className} has-media" data-open-image="${item.image}" data-open-title="${item.title}" data-open-text="${item.text}" tabindex="0" role="button">
      <div class="card-media">
        <img src="${item.image}" alt="${item.title}" loading="lazy" />
        <span class="zoom-chip">${content[currentLang].clickToZoom}</span>
      </div>
      <h3>${item.title}</h3>
      <p>${item.text}</p>
      <span class="cert-meta">${item.meta || content[currentLang].clickToZoom}</span>
    </article>
  `;
}
...
function renderGallery(lang) {
  galleryGrid.innerHTML = content[lang].gallery.map(item => `
    <article class="gallery-card has-media ${item.featured ? "featured" : ""}" data-open-image="${item.image}" data-open-title="${item.title}" data-open-text="${item.text}" tabindex="0" role="button">
      <div class="gallery-media">
        <img src="${item.image}" alt="${item.title}" loading="lazy" />
        <span class="zoom-chip">${content[lang].clickToZoom}</span>
      </div>
      <h3>${item.title}</h3>
      <p>${item.text}</p>
    </article>
...
function renderFaq(lang) {
  faqList.innerHTML = content[lang].faq.map((item, index) => `
...
function renderAll() {
  ...
  renderMethods(currentLang);
  renderServices(currentLang);
  renderCertificates(currentLang, currentCertFilter);
...
document.addEventListener("click", (event) => {
  ...
  if (card) {
    openLightbox({
      image: card.dataset.openImage,
      title: card.dataset.openTitle,
      text: card.dataset.openText
    });
    return;
  }
...
certTabs.addEventListener("click", (event) => {
  const button = event.target.closest("button[data-cert-filter]");
  if (!button) return;
